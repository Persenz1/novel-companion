...
- metric_change（数值变化）: { id, series_id, metric_id, old_value, new_value, delta, reason, valid_from, visible_from, source_span, status:"accepted" }（不知道具体值就不要编）
...
- id 用可读 slug（实体如 entity_xxx、术语如 term_xxx），同一实体复用已确认列表里的 id。
- 宁可少抽几条高质量的，也不要堆大量低质量候选。
...
3. 是否属高风险类别（必须升级给人，route=escalate）：
   - 实体合并 / 疑似同一实体
