...
本文件保留 Agent 的职责边界、工具接口、安全边界和审计原则，但暂不作为阶段 5-8 的直接实现清单。

2026-06-30 曾尝试实现一个包含候选生成、Web 工作台、预览候选、逐条接受/拒绝/转未决的原型。交互验证后确认：candidate-by-candidate 的人工复核方式会导致真实长篇制作工作量过大，且人类操作集中在低层微决策上。因此该实现已回滚，阶段 5-8 操作逻辑待重构。

在新的操作逻辑定案前：

- 不应继续实现逐候选卡片式工作台。
- 不应把本文件第 6、9、11 节直接当作 UI/CLI 验收清单。
- 仍必须遵守 AI 不得静默写 Accepted、Accepted 必须人工确认并生成 Change、正式数据必须可追溯等硬边界。
- 新设计应优先考虑 block/range 级批处理、异常队列、差异视图、人工批注和 Agent 结构化落盘。
...
当前注意：以下循环是早期推荐模型，已被标记为需要重构；特别是“等待人工确认”需要重新定义为低摩擦的批处理/批注/异常裁决，而不是每条候选一个按钮。
...
当前注意：本节验收标准已暂停作为实现验收使用。待阶段 5-8 操作逻辑重构完成后，应更新本节。
