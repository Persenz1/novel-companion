...
当前注意：本测试书仍作为格式和工具链验收夹具，但不再证明逐候选工作台是正确交互。Candidates、Review、Accepted 的计数要求主要用于 fixture、validator、compiler 和 query 测试；真实阶段 5-8 工作台操作逻辑待重构。
...
说明：这些覆盖点用于保证数据格式和受控写入能力存在，不要求最终工作台以逐候选点击方式完成所有动作。
