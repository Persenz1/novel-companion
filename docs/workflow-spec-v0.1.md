...
但第 3-5 节中关于 Agent、候选生成和人工复核的最小操作流已经经过一次原型验证，结论是逐候选点击式工作台不适合作为真实长篇制作流程。阶段 5-8 当前暂停继续实现，等待操作逻辑重构。详见 `docs/phase-5-8-operation-redesign-note.md`。
...
当前注意：Agent 的职责是降低上下文搬运和结构化落盘成本，不是把所有 AI 输出拆成大量候选卡片交给人工逐条点击。新的操作流应优先考虑 block/range 级批处理建议、异常队列和差异确认。
...
当前注意：Candidate 仍是重要中间格式，但不应把“候选数量越多”当作工作台目标。后续设计需要明确哪些候选应聚合、哪些应批量确认、哪些必须单条人工裁决。

## 5. 人工复核

人工复核以 block 为最小单位，按正文顺序推进。

当前注意：本节的逐项操作列表是早期最小闭环设想，已经不足以支撑真实长篇制作。新的复核设计应从“人类在 block/range 上做裁决、批注和异常处理”出发，而不是从“人类逐条处理 Candidate”出发。
