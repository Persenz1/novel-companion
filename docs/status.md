...
- 起草 / 复核流水线 v2：`tools/src/agent/pipeline.ts` + `prompts.ts`。「卷 + pass」运行，当前实际启用 3 个 pass（1 实体名册 / 2 事实数值术语 / 3 事件关系；speakers 密集标注 pass 代码保留但已停用，见下），窗口对齐 separator/章节边界、稳定前缀（全卷正文+已确认记忆在前，变化区仅 ~0.5%）吃前缀缓存、JSONL 容截断输出 + 自动续写、关系变化代码级强制升级、draft-pass resume-skip（重跑跳过同 volume/pass 已有 completed work_run 的窗口）。入口：工作台 pass 选择器、CLI `draft-pass` / `review-pass`、`POST /api/draft|review {volume_id, pass}`。设计与 v1 缺陷实据见 `modules/drafting-review-v2-design.md`。**2026-07-03 已用 DeepSeek 对真实 COTE 三卷（v01-v03）全量实跑验证**：entities/knowledge/narrative 三 pass 全部 draft+review 完成，全包 validate+compile passed，缓存命中率均值 95.9%、relation_change 66 条 100% 强制升级、复核在三态（auto/escalate/reject）上都有真实判断力。过程中发现并修复 6 个数据完整性 bug（entities 信封 type 冲突、实体 id 空格、visible_from/source_span 幻觉与重复字段设计、`edited_draft` 字符串化崩溃、metric_change 数值字符串化、fact.valid_from 幻觉引用）+ 补齐 resume-skip；详见 `modules/drafting-review-v2-three-volume-test-2026-07-03.md`。speakers pass 在 v01 实测覆盖率仅 81.3%（3 轮补跑仍缺 402/2154 对话块）且成本超过其余三 pass 总和，用户判断放弃，产出已回滚清除，不建议短期内重新启用。
...
- 真实 COTE 三卷清洗 + 起草 + 复核已在本机数据包 `~/nc-workpack/cote-cleaning-panel-recovered-2026-07-02` 端到端跑通：
  ...
  - 已知风险：当前 v02/v03 对照主要按章节强制重划分后顺序匹配，能把错位限制在章节内，但仍可能有串行 / 一对多 / 多对一结构差异；已写 `review/ja_alignment_items.jsonl`，暂不作为优化重点。
  - v02/v03 MiMo 清洗：14 个正文章节完成，36 条低风险建议全部应用；v02/v03 正文图片缺图注为 0，锚点有效。
  - **DeepSeek 起草 / 复核 v2（2026-07-03，全量三卷）**：entities/knowledge/narrative 三 pass 全部 draft+review 完成，最终 `validate` + `compile` passed。最终 accepted：entities 83、facts 349、events 173、metrics 31、metric_changes 4、term_cards 6；review item 待人工裁决 192 条（还没做）；relation_change 66 条 100% 强制升级、0 自动落盘。起草使用 `deepseek-v4-flash`，复核使用 `deepseek-v4-pro`。speakers pass 已放弃（见上）。详细过程、暴露的 6 个 bug 和结果表见 `modules/drafting-review-v2-three-volume-test-2026-07-03.md`。
  - 当前结论：清洗、匹配、起草、复核三 pass 已在真实三卷规模上验证稳定，可作为后续阅读器作业基线；下一步是 192 条 review item 的人工裁决，而不是继续测起草/复核本身。
...
真实商业 EPUB 测试语料放在 `samples/real-epubs/`（本机，`*.epub` 已被 `.gitignore` 忽略，正文不入库）。只有测试登记表 + 预期基线 + 已知怪癖入库，见 `modules/real-epub-test-corpus.md` 与 `modules/compatibility-testing-plan.md`。当前语料：COTE 中译 v01/v02/v03 与日文原版 v01-v03；本轮结果固化到 `~/nc-workpack/cote-cleaning-panel-recovered-2026-07-02`（`cote-bilingual-v1` 等旧工作副本已删除），真实书正文和模型输出不提交入库。
...
- 工作台作业粒度已从章节改为「卷 + pass + 自动窗口」（v2）；任意 scene / block range 的手动指定仍未支持。v2 三卷全量实跑已验证（见上），已取代旧 v1 产出（v1 的密度问题——6143 对话块仅 1 条说话人、relation_change 为 0、后卷密度崩塌——是 v2 立项的原因，v2 结果不再需要与 v1 对照）。resume-skip 已实现（重跑跳过已完成窗口），但只覆盖 `runDraftPass`；`resolveExceptionsBatch`（人工裁决路径）尚未做同等的 block 引用合法性校验，valid_from/valid_until 幻觉理论上仍可能通过这条路径漏进 accepted。
