...
当前设计里，中文正文是唯一主轴；日文不建立独立 timeline，也不驱动 Candidate / Accepted 生成。

日文可有两种再生产物：

- `source/ja/{volume}.blocks.json`：`block_id -> 日文正文`，供阅读器逐段显示。
- `source/ja/{volume}.json`：`alignment_id -> { confidence, ja_refs[] }`，由 Parser 合并到 `parsed/alignments.jsonl`，供 query 在 `includeJa` 时返回 reviewed 日文参考。

真实书籍主线测试优先把 `source/ja/{volume}.blocks.json` 作为阅读器主产物；alignment 队列用于审计一对多、多对一、低置信和未匹配。日文原文不得直接生成实体、事实、事件、关系、数值或说话人标签。
