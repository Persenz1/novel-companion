...
状态：已实现（含无模型单测 10 例），2026-07-02 用 DeepSeek 对 v01 实跑部分验证（entities / knowledge / narrative 全流程 + speakers 前 2 窗，详见 §10 实测进展）。取代旧的「每章一次调用、全类型混抽、最多 15 条」起草复核形态。
...
1. v01 四 pass 全量重跑（draft + review × entities/knowledge/narrative/speakers，数据已回滚，从 entities 起）。
2. speakers 完整 22 窗 + 覆盖率终值（目标接近 100%）+ speakers review（ambiguous 升级抽查）。
3. v01 整体评估：密度 / 覆盖率 / 缓存命中率 / relation_change 升级率，与 v1 基线（accepted 283）同卷对比。
4. v02、v03 各四 pass draft + review（验证跨卷记忆传递 + 后卷密度不衰减）。
5. 工程补强：pass 级 resume-skip（重跑跳过已完成窗口），避免中断续跑重复产候选。
