...
当前注意：本测试书仍作为格式和工具链验收夹具，但不再证明逐候选工作台是正确交互。仓库中的 `samples/gray-tower` 提交态保持在“清洗后、无分析数据”状态；测试所需 Candidates / Review / Accepted 由确定性 fixture 临时生成，不调用真实大模型，也不需要 API key。
...
说明：这些覆盖点用于保证数据格式和受控写入能力存在，不要求最终工作台以逐候选点击方式完成所有动作；真实工作台按 v0.2 用起草/复核/异常队列/审计回滚闭环。
...
说明：本节是样例数据覆盖要求，不是最终人机交互验收要求。提交态样例包不保存这些分析结果，集成测试会复制清洗后样例包到临时目录再生成。
...
- 至少 1 个 `asset_subject` 确认写入。
- 至少 1 个图片人物识别候选进入 Review，不自动 Accepted（图片人物身份属 v0.2 高风险升级项）。
