...
图片用 `asset` 注释锚定到某个 block。`alt` 是图注（图里是谁/是什么）。

> 更新（2026-07）：图片内容标注现在**在清洗阶段完成并定死**——工作台的「清洗·图片」页用多模态模型（如 MiMo）看图、给出 `alt` + 描述，人工确认后写回本标记的 `alt` 并重解析。下游操作阶段（纯文本）直接信任该 `alt`，不再在操作阶段临时判定「图里是谁」。清洗仍必须提供锚点；`alt` 可留空由该页补全。
